...
# Export dir -------------------------------------------------------------------
export_dir <- here("results", "processed_data", "supply_curves", "with_mpas_abt_all_cells")

# Load data --------------------------------------------------------------------
master_data <- readRDS(
  file = here("results",
              "processed_data",
              "master_costs_and_benefits_abt_all_cells.rds")) %>% 
  replace_na(replace = list(suitability = 0))
...
saveRDS(object = eez_supply_curves_w_mpas,
        file = here(export_dir, "global_eez_supply_curves_with_mpas_abt_all_cells.rds"))
# Export horizontally summed
saveRDS(object = global_supply_curve_w_mpas,
        file = here(export_dir, "global_supply_curve_with_mpas_abt_all_cells.rds"))

# Export hemisphere-level data -------------------------------------------------
saveRDS(object = hemisphere_eez_supply_curves_w_mpas,
        file = here(export_dir, "hemisphere_eez_supply_curves_with_mpas_abt_all_cells.rds"))
# Export horizontally summed hemisphere data
saveRDS(object = hemisphere_supply_curves_w_mpas,
        file = here(export_dir, "hemisphere_supply_curves_with_mpas_abt_all_cells.rds"))

# Export realm level data ------------------------------------------------------
saveRDS(object = realm_eez_supply_curves_w_mpas,
        file = here(export_dir, "realm_eez_supply_curves_with_mpas_abt_all_cells.rds"))
# Export horizontally summed realms
saveRDS(object = realm_supply_curves_w_mpas,
        file = here(export_dir, "realm_supply_curves_with_mpas_abt_all_cells.rds"))

# Export province level data ---------------------------------------------------
saveRDS(object = province_eez_supply_curves_w_mpas,
        file = here(export_dir, "province_eez_supply_curves_with_mpas_abt_all_cells.rds"))
# Export horizontally summed province level data
saveRDS(object = province_supply_curves_w_mpas,
        file = here(export_dir, "province_supply_curves_with_mpas_abt_all_cells.rds"))

# Export ecoregion level data --------------------------------------------------
saveRDS(object = ecoregion_eez_supply_curves_w_mpas,
        file = here(export_dir, "ecoregion_eez_supply_curves_with_mpas_abt_all_cells.rds"))
# Export horizontally summed province level data
saveRDS(object = ecoregion_supply_curves_w_mpas,
        file = here(export_dir, "ecoregion_supply_curves_with_mpas_abt_all_cells.rds"))
