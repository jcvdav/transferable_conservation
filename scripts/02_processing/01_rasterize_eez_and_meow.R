...
writeRaster(x = eez_raster,
            filename = file.path(project_path, "data", "eez_raster.tif"),
            overwrite = TRUE)
writeRaster(x = realm_raster,
            filename = file.path(project_path, "data", "rlm_raster.tif"),
            overwrite = TRUE)
writeRaster(x = province_raster,
            filename = file.path(project_path, "data", "pro_raster.tif"),
            overwrite = TRUE)
writeRaster(x = ecoregion_raster,
            filename = file.path(project_path, "data", "eco_raster.tif"),
            overwrite = TRUE)
