...
# Source directory -------------------------------------------------------------
source_dir <- here("results", "processed_data", "supply_curves", "with_mpas_abt_all_cells")


# Load data --------------------------------------------------------------------
eez_cb <- readRDS(file = here(source_dir, "global_eez_supply_curves_with_mpas_abt_all_cells.rds"))
eez_h_sum_cb  <- readRDS(file = here(source_dir, "global_supply_curve_with_mpas_abt_all_cells.rds"))

hem_eez_cb <- readRDS(file = here(source_dir, "hemisphere_eez_supply_curves_with_mpas_abt_all_cells.rds"))
hem_h_sum <- readRDS(file = here(source_dir, "hemisphere_supply_curves_with_mpas_abt_all_cells.rds"))

rlm_eez_cb <- readRDS(file = here(source_dir, "realm_eez_supply_curves_with_mpas_abt_all_cells.rds"))
rlm_h_sum <- readRDS(file = here(source_dir, "realm_supply_curves_with_mpas_abt_all_cells.rds"))

pro_eez_cb <- readRDS(file = here(source_dir, "province_eez_supply_curves_with_mpas_abt_all_cells.rds"))
pro_h_sum <- readRDS(file = here(source_dir, "province_supply_curves_with_mpas_abt_all_cells.rds"))

eco_eez_cb <- readRDS(file = here(source_dir, "ecoregion_eez_supply_curves_with_mpas_abt_all_cells.rds"))
eco_h_sum <- readRDS(file = here(source_dir, "ecoregion_supply_curves_with_mpas_abt_all_cells.rds"))
  ...
  function(r, curves, agg_curves, group, write = F) {
    ...
    realized_mkt_cb <- curves %>%
      ...
      summarize(
        ...
        mkt_tb_hsi = sum(adj_benefit_hsi, na.rm = T),
    ...
    gains_from_trade <- combined_outcomes %>%
      select(bau_area, mkt_area, bau_tb, mkt_tb, bau_tb_hsi, mkt_tb_hsi, bau_tc, difference = savings) %>%
...
saveRDS(
  ...
  file = here("results", "output_data", "gains_from_trade_bubbles_abt_all_cells.rds")
)
