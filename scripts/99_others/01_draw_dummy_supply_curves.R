...
p1 <- data %>% 
  filter(n == "B") %>% 
  ggplot() +
  geom_line(mapping = aes(x = q / 10, y = p), size = 1, color = "orange") +
...
p2 <- p1 +
  ...
  geom_segment(x = 0, xend = 0.1, y = 2, yend = 2, linetype = "dashed", color = "orange") +
...
p3 <- p1 +
  ...
  geom_segment(x = 0, xend = 0.3, y = 6, yend = 6, linetype = "dashed", color = "orange") + 
...
p4 <- p_full +
  ...
  geom_segment(aes(x = 0, xend = 0.3, y = 6, yend = 6), linetype = "dashed", color = "orange") +
...
p4_area <- p4 +
  ...
  geom_text(data = bau_text %>% head(2),
...
p6 <- p_full  +
  geom_vline(xintercept = 0.3, linetype = "dashed", color = "gray") +
  geom_segment(aes(x = 0, xend = 0.4, y = 4, yend = 4), linetype = "dashed", color = "black") +
  geom_segment(aes(x = 0.2, xend = 0.2, y = 0, yend = 4), color = "orange", linetype = "dashed") +
...
p6_area <- p6 +
  ...
  geom_polygon_pattern(data = tibble(x = c(0.3, 0.3, 0.4, 0.4, 0.3),
               ...
               color = "transparent",
               alpha = 0.5) +
  geom_polygon(data = pol2 %>% filter(n == "B"), aes(x = q, y = p), fill = "orange", color = "black") +
  ...
  geom_text(data = mkt_text %>% head(2), aes(x = x, y = y, label = label), parse = T) 
...
p7 <- p6 +
  geom_polygon(data = pol2 %>% filter(n == "B"), aes(x = q, y = p), fill = "orange", alpha = 0.5, color = "transparent") +
...
p8 <- ggplot(data = sc_dat, aes(x = tb, y = mc)) +
  geom_step(direction = "vh", size = 1, color = "orange") +
  geom_text(data = sc_dat %>% filter(pixel > 0), aes(label = paste("Pixel", pixel)), nudge_x = -3, nudge_y = 0.5) +
  theme_half_open() +
  labs(x = "Conservation (Q)", y = expression("Marginal Costs ("~c[i]~"/"~q[i]~")"))
